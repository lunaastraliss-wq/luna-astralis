...
const offers = [
  {
    ...
    features: ["Soleil, Lune et Ascendant", "Les 10 planètes", "Résumé personnalisé", "PDF téléchargeable"],
  },
  {
    ...
    featured: true,
    features: ["Tout le rapport Essentielle", "Les 12 maisons astrologiques", "Amour, carrière et finances", "Forces, défis et mission de vie", "PDF détaillé"],
  },
  {
    ...
    features: ["Tout le rapport Premium", "Aspects astrologiques majeurs", "Chiron et nœuds lunaires", "Dominantes du thème", "Analyse haut de gamme"],
  },
];
  ...
  return (
    <section className="natal-premium-offer">
      ...
      <p className="natal-premium-intro">
        Trois niveaux d'analyse personnalisée selon votre date, votre heure et votre lieu de naissance.
      </p>

      <div className="natal-offers">
        {offers.map((offer) => (
          <div key={offer.key} className={offer.featured ? "natal-offer-card featured" : "natal-offer-card"}>
            {offer.featured && <div className="best-value">⭐ Le plus populaire</div>}
            ...
            <button type="button" className="natal-premium-btn" onClick={() => handleCheckout(offer.key)}>
              {offer.button}
            </button>
          </div>
        ))}
