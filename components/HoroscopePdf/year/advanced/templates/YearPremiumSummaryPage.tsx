...
function resolvePageIcon(
  ...
  return {
    iconKey,
    icon: getIcon(iconKey, "integrationGuide"),
  };
}

function resolveSummaryItemIconKey(
  ...
  return (
    inferIconKeyFromText(item.label) ||
    inferIconKeyFromText(item.title) ||
    inferIconKeyFromText(item.text) ||
    inferIconKeyFromText(item.secondaryText) ||
    fallbackKey
  );
}
