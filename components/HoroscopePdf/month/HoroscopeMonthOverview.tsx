...
function getMonthlyClimate(value: number) {
  if (value >= 80) {
    return {
...
function buildFocusItems(
  ...
  const climate = getMonthlyClimate(overallScore);
...
export default function HoroscopeMonthOverview({
  identity,
  period,
  content,
}: HoroscopeSectionProps) {
  ...
  const monthlyClimate = getMonthlyClimate(overallScore);
  const focusItems = buildFocusItems(scores, overallScore);

  const summaryTitle =
    content.summary.title ||
    monthlyClimate.title;
