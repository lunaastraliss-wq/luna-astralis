export type PremiumPlanetEvolutionContent = {
  text: string;
};

export type PremiumPlanetEvolutionTexts = Record<
  string,
  Record<string, PremiumPlanetEvolutionContent>
>;

export const PREMIUM_PLANET_EVOLUTION: PremiumPlanetEvolutionTexts = {
  Sun: {
    Aries: {
      text:
...
export function getPremiumPlanetEvolution(
  planet: string,
  sign: string
): string {
  const planetEvolution =
    PREMIUM_PLANET_EVOLUTION[
      planet
    ];
