...
const HOUSE_MEANINGS: Record<number, string> = {
  1: "l’identité, l’apparence et la manière d’entrer en contact avec le monde",
  2: "les ressources, la sécurité matérielle et les valeurs personnelles",
  3: "la communication, les apprentissages et l’environnement immédiat",
  4: "les racines, le foyer, la vie privée et la sécurité émotionnelle",
  5: "la créativité, les plaisirs, l’expression personnelle et les amours",
  6: "le travail quotidien, les habitudes, le service et l’organisation",
  7: "les relations, les partenariats et la recherche d’équilibre",
  8: "les transformations, l’intimité, les crises et les ressources partagées",
  9: "les croyances, les voyages, les études et l’élargissement de la conscience",
  10: "la vocation, la carrière, la réputation et les ambitions",
  11: "les projets collectifs, les amitiés et la vision de l’avenir",
  12: "la vie intérieure, l’inconscient, le retrait et la spiritualité",
};
...
function createElementCounts(): CountMap<ElementName> {
  return {
    Feu: 0,
    Terre: 0,
    Air: 0,
...
function createModalityCounts(): CountMap<ModalityName> {
  return {
    Cardinal: 0,
    Fixe: 0,
    Mutable: 0,
...
function calculateDominant<T extends string>(
...
function calculateModalities(
  ...
  const counts = createModalityCounts();
...
function getDominantPlanet(
  ...
  return planets.reduce(
    (bestPlanet, currentPlanet) => {
      const bestScore = getPlanetScore(
        bestPlanet,
        angles
      );

      const currentScore = getPlanetScore(
        currentPlanet,
        angles
      );
...
function createGlobalSynthesis(
  ...
  const houseText =
    dominantHouse
      ? `La maison ${dominantHouse}, liée à ${
          HOUSE_MEANINGS[dominantHouse]
        }, devient également un territoire important de votre évolution.`
      : "";
...
export default function PdfDominants({
  planets,
  angles,
}: PremiumPdfSummaryProps) {
  const safePlanets = getValidPlanets(
  ...
  const elementCounts =
    calculateElements(safePlanets);

  const modalityCounts =
    calculateModalities(safePlanets);

  const dominantElement =
    calculateDominant(elementCounts);

  const dominantModality =
    calculateDominant(modalityCounts);
  ...
  const totalModalities = Object.values(
    modalityCounts
  ).reduce(
    (sum, count) => sum + count,
    0
    ...
    <>
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.intro}>
            Les dominantes révèlent les énergies qui
            reviennent le plus fortement dans votre
            thème natal. Elles forment la tonalité
            générale de votre personnalité et montrent
            les mécanismes que vous utilisez le plus
            spontanément.
          </Text>
          ...
          <View style={styles.grid}>
            <View style={styles.halfCardLeft}>
              ...
              <Text style={styles.dominantName}>
                {dominantElement.name}
              </Text>

              <Text style={styles.dominantScore}>
                {dominantElement.percentage} % des
                planètes analysées
              </Text>
            ...
            <View style={styles.halfCardRight}>
              ...
              <Text style={styles.dominantScore}>
                {dominantModality.percentage} % des
                planètes analysées
              </Text>
          ...
          <View style={styles.distributionCard}>
            <Text
              style={styles.distributionTitle}
            >
      ...
      <Page size="A4" style={styles.page}>
        ...
        <View style={styles.content}>
          ...
          <Text style={styles.intro}>
            Certaines planètes et certaines maisons
            occupent une place stratégique dans votre
            carte du ciel. Elles indiquent les domaines
            dans lesquels votre énergie se concentre
            naturellement.
          </Text>
          ...
          <View style={styles.featureCard}>
            <View style={styles.featureHeader}>
              ...
              <Text style={styles.featureTag}>
                {dominantHouse.count > 0
                  ? `${dominantHouse.count} planète${
          ...
          <View style={styles.featureCard}>
            ...
            <Text style={styles.featureText}>
              Votre élément dominant décrit la
              substance principale de votre énergie,
              tandis que votre modalité dominante
              montre la manière dont vous la mettez en
              mouvement. Ensemble, ils révèlent votre
              rythme naturel et votre façon habituelle
