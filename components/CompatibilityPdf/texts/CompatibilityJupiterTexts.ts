...
export const JUPITER_SAME_SIGN_TEXTS: CompatibilityJupiterTextTemplate[] = [
  ...
  ({ sign1 }) =>
    `Vous exprimez tous les deux votre besoin d’expansion à travers l’énergie du ${sign1}. Cette ressemblance facilite les projets communs, les voyages, les apprentissages ou les expériences qui enrichissent votre relation. Elle demande toutefois de garder un certain réalisme afin de concrétiser vos ambitions.`,
...
export const JUPITER_SAME_ELEMENT_TEXTS: CompatibilityJupiterTextTemplate[] = [
  ...
  ({ sign1, sign2, element1 }) =>
    `Le ${sign1} et le ${sign2} expriment deux facettes différentes d’une même énergie d’expansion, celle du ${element1}. Cette proximité vous aide à construire des projets solides et à maintenir une vision optimiste.`,

  ({ sign1, sign2, element1 }) =>
    `La compatibilité entre vos Jupiter repose sur leur élément commun, le ${element1}. Vous partagez des valeurs compatibles et une manière semblable d’aborder les opportunités qui se présentent à vous.`,
...
export const JUPITER_COMPLEMENTARY_ELEMENT_TEXTS: CompatibilityJupiterTextTemplate[] =
  [
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `${sign1} recherche l’expansion à travers le ${element1}, tandis que ${sign2} construit sa confiance selon le ${element2}. Cette complémentarité peut vous aider à transformer vos idées en expériences concrètes et vos ambitions en projets réalisables.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `Votre duo jupitérien réunit le ${element1} et le ${element2}. Cette combinaison crée un équilibre intéressant entre vos manières de prendre des risques, de saisir les occasions et de donner du sens à vos expériences.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `Vos Jupiter associent deux éléments capables de se renforcer : le ${element1} et le ${element2}. L’un peut ouvrir de nouvelles possibilités, tandis que l’autre leur donne une direction, une profondeur ou une forme plus durable.`,

    ({ sign1, sign2, element1, element2 }) =>
      `La complémentarité entre Jupiter en ${sign1} et Jupiter en ${sign2} repose sur l’alliance du ${element1} et du ${element2}. Vos différences de vision deviennent une force lorsque chacun soutient l’expansion de l’autre sans chercher à la contrôler.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Les éléments ${element1} et ${element2} donnent à vos Jupiter des qualités qui peuvent s’équilibrer dans les projets communs. Le ${sign1} peut inspirer une possibilité que le ${sign2} aide à développer, structurer ou approfondir.`,

    ({ sign1, sign2, element1, element2 }) =>
      `L’association du ${element1} et du ${element2} favorise une dynamique où vos ambitions peuvent se compléter. Jupiter en ${sign1} et Jupiter en ${sign2} deviennent particulièrement compatibles lorsque vous considérez vos différences comme des ressources pour grandir ensemble.`,
  ];
...
export const JUPITER_DIFFERENT_ELEMENT_TEXTS: CompatibilityJupiterTextTemplate[] =
  [
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `${sign1} développe sa confiance à travers le ${element1}, tandis que ${sign2} recherche l’expansion selon le ${element2}. Vous ne saisissez pas toujours les mêmes occasions et pouvez avoir des visions distinctes de ce qui constitue une réussite.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Vos Jupiter réunissent le ${element1} et le ${element2}. Cette combinaison demande de l’adaptation, car chacun possède sa propre manière de prendre des risques, d’apprendre de ses expériences et de construire son avenir.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `Entre le ${sign1} et le ${sign2}, les ambitions peuvent suivre des directions distinctes. Le ${element1} et le ${element2} ne donnent pas toujours la même importance à la sécurité, à l’aventure, à l’émotion ou à la réflexion, ce qui demande une véritable écoute.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Vos philosophies de vie ne parlent pas spontanément le même langage. Jupiter lié au ${element1} peut considérer une possibilité comme essentielle, tandis que Jupiter associé au ${element2} la juge irréaliste, prématurée ou peu significative.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Jupiter en ${sign1} et Jupiter en ${sign2} possèdent des façons distinctes de rechercher la confiance et l’accomplissement. Cette différence entre le ${element1} et le ${element2} peut créer des frustrations, mais elle peut aussi empêcher votre couple de rester enfermé dans une seule vision.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `Jupiter en ${sign1} peut poursuivre une forme d’expansion que Jupiter en ${sign2} ne comprend pas immédiatement. Cette différence entre le ${element1} et le ${element2} demande de clarifier vos objectifs afin d’éviter que l’un se sente freiné ou que l’autre se sente entraîné trop loin.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Vos chemins d’évolution suivent des directions distinctes en raison de l’association du ${element1} et du ${element2}. Le ${sign1} et le ${sign2} peuvent néanmoins bâtir un avenir enrichissant lorsqu’ils apprennent à faire coexister leurs ambitions plutôt qu’à déterminer laquelle est la plus valable.`,
  ];
