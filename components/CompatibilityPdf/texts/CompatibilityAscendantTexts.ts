...
export const ASCENDANT_SAME_SIGN_TEXTS: CompatibilityAscendantTextTemplate[] = [
  ...
  ({ sign1 }) =>
    `Vous exprimez tous les deux votre personnalité visible à travers l’énergie ${sign1}. Cette proximité favorise une belle coordination dans vos habitudes et vos interactions sociales. Elle demande néanmoins de laisser à chacun la liberté de développer son propre style.`,
  ...
  ({ sign1 }) =>
    `Vos façons d’entrer dans une situation, d’observer les autres et de protéger votre espace personnel sont influencées par le même signe, ${sign1}. Cette ressemblance peut créer une impression de compréhension immédiate, parfois même avant que les mots soient nécessaires.`,

  ({ sign1 }) =>
    `Lorsque vos deux Ascendants occupent le signe ${sign1}, vous partagez une manière familière de vous montrer, de vous adapter et de prendre votre place. Cette proximité peut renforcer la fluidité de votre relation, à condition de ne pas tomber dans une dynamique trop prévisible.`,
...
export const ASCENDANT_SAME_ELEMENT_TEXTS: CompatibilityAscendantTextTemplate[] =
  [
    ...
    ({ sign1, sign2, element1 }) =>
      `${sign1} et ${sign2} expriment deux facettes de l’élément ${element1}. Vos attitudes peuvent différer, mais elles reposent sur une dynamique similaire qui facilite la compréhension et la coordination.`,

    ({ sign1, sign2, element1 }) =>
      `La compatibilité entre vos Ascendants repose sur leur élément commun, ${element1}. Vous pouvez avancer avec un rythme semblable et partager une manière comparable de vous présenter, de vous protéger et de répondre à votre environnement.`,
    ...
    ({ sign1, sign2, element1 }) =>
      `Grâce à leur appartenance commune à l’élément ${element1}, vos Ascendants possèdent des réflexes compatibles. ${sign1} et ${sign2} apportent chacun une nuance différente, mais ils partagent une manière cohérente d’aborder la vie.`,
  ];
...
export const ASCENDANT_COMPLEMENTARY_ELEMENT_TEXTS: CompatibilityAscendantTextTemplate[] =
  [
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `${sign1} aborde le monde à travers ${element1}, tandis que ${sign2} réagit selon ${element2}. Cette complémentarité peut aider chacun à développer une manière plus souple et plus complète de s’adapter.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `Votre duo d’Ascendants réunit ${element1} et ${element2}. Cette association peut créer un bel équilibre entre vos façons d’entrer en action, de vous adapter et de vous présenter aux autres.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `Vos Ascendants associent deux éléments capables de se renforcer : ${element1} et ${element2}. L’un peut apporter l’élan, la stabilité, l’ouverture ou l’adaptabilité dont l’autre a besoin selon les circonstances.`,

    ({ sign1, sign2, element1, element2 }) =>
      `La complémentarité entre les Ascendants en ${sign1} et en ${sign2} repose sur l’alliance ${element1} et ${element2}. Vos différences peuvent devenir une force lorsque chacun reconnaît la valeur du style naturel de l’autre.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Les éléments ${element1} et ${element2} donnent à vos Ascendants des qualités qui peuvent se compléter dans le quotidien. ${sign1} peut ouvrir une voie que ${sign2} consolide, tandis que chacun apporte une manière distincte de réagir.`,

    ({ sign1, sign2, element1, element2 }) =>
      `L’association ${element1} et ${element2} crée une dynamique où vos comportements extérieurs peuvent s’équilibrer. ${sign1} et ${sign2} deviennent complémentaires lorsque vous laissez vos différences contribuer à la relation plutôt que de chercher à les corriger.`,
  ];
...
export const ASCENDANT_DIFFERENT_ELEMENT_TEXTS: CompatibilityAscendantTextTemplate[] =
  [
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `${sign1} réagit à travers ${element1}, tandis que ${sign2} s’adapte selon ${element2}. Vous ne lisez pas toujours les situations de la même façon, mais cette différence peut élargir votre manière de comprendre le monde.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Vos Ascendants réunissent ${element1} et ${element2}. Cette combinaison demande davantage d’ajustement, car chacun possède sa propre manière de réagir, de se protéger et de prendre sa place.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `Entre ${sign1} et ${sign2}, les différences de comportement peuvent être visibles. ${element1} et ${element2} n’adoptent pas spontanément le même rythme, ce qui demande patience et compréhension.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Vos manières de vous présenter et de vous adapter ne parlent pas toujours le même langage. L’Ascendant lié ${element1} peut réagir d’une façon que celui associé ${element2} comprend difficilement au premier abord.`,

    ({ sign1, sign2, element1, element2 }) =>
      `L’Ascendant en ${sign1} et celui en ${sign2} possèdent des réflexes distincts. Cette différence entre ${element1} et ${element2} peut provoquer des tensions dans le quotidien, mais aussi enrichir votre capacité d’adaptation.`,
    ...
    ({ sign1, sign2, element1, element2 }) =>
      `L’Ascendant en ${sign1} peut privilégier une approche que celui en ${sign2} ne choisit pas spontanément. Cette différence entre ${element1} et ${element2} demande une certaine flexibilité afin d’éviter les malentendus.`,

    ({ sign1, sign2, element1, element2 }) =>
      `Vos comportements extérieurs suivent des logiques différentes en raison de l’association ${element1} et ${element2}. ${sign1} et ${sign2} peuvent apprendre à mieux se comprendre en observant leurs réactions sans chercher à les uniformiser.`,
  ];
