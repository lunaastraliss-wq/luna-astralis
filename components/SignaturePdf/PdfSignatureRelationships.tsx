...
type SignatureRelationshipPlanet = {
  name?: string;
  sign?: string;
  degree?: number;
  longitude?: number;
...
type SignatureRelationshipAngles = {
  ascendant?: number;
  midheaven?: number;
  descendant?: number;
  imumCoeli?: number;
...
type PdfSignatureRelationshipsProps = {
  planets?: SignatureRelationshipPlanet[];
  angles?: SignatureRelationshipAngles;
};
...
> = {
  ...
  Taureau:
    "Vous avez besoin de stabilité, de constance et de preuves concrètes d’affection. La confiance se construit progressivement grâce à la fiabilité, à la sensualité et à la présence quotidienne.",
...
): SignatureRelationshipPlanet | null {
  return (
    planets.find((planet) => {
      return (
...
function getPlanetHouseText(
  planet: SignatureRelationshipPlanet | null
): string {
  if (!planet) {
    return "";
  }
...
export default function PdfSignatureRelationships({
  planets,
  angles,
}: PdfSignatureRelationshipsProps) {
  const safePlanets =
