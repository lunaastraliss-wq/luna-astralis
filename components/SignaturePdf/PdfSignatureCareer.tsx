...
type PdfSignatureCareerProps = {
  planets?: SignatureCareerPlanet[];
  angles?: SignatureCareerAngles;
};
...
const SIGN_CAREER_TEXTS: Record<string, string> = {
  Bélier:
...
const HOUSE_CAREER_TEXTS: Record<number, string> = {
  1:
...
const MC_SIGN_TEXTS: Record<string, string> = {
  Bélier:
...
function getHouseText(
  planet: SignatureCareerPlanet | null
): string {
  if (!planet) {
    return "";
  }
...
function getCareerStrength(
  sun: SignatureCareerPlanet | null,
  jupiter: SignatureCareerPlanet | null
): string {
  const sunSign =
    getFrenchSign(
      sun?.sign
...
function getCareerChallenge(
  mars: SignatureCareerPlanet | null,
  saturn: SignatureCareerPlanet | null
): string {
  const marsSign =
    getFrenchSign(
      mars?.sign
...
function createCareerSynthesis(
  sun: SignatureCareerPlanet | null,
  mercury: SignatureCareerPlanet | null,
  mars: SignatureCareerPlanet | null,
  saturn: SignatureCareerPlanet | null,
...
export default function PdfSignatureCareer({
  planets,
  angles,
}: PdfSignatureCareerProps) {
  const safePlanets =
