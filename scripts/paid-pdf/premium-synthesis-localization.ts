...
function createDominantText(
  ...
  const elementNames: Record<string, string> = ${JSON.stringify(({"en": {"Feu": "Fire", "Terre": "Earth", "Air": "Air", "Eau": "Water"}, "es": {"Feu": "Fuego", "Terre": "Tierra", "Air": "Aire", "Eau": "Agua"}, "de": {"Feu": "Feuer", "Terre": "Erde", "Air": "Luft", "Eau": "Wasser"}, "it": {"Feu": "Fuoco", "Terre": "Terra", "Air": "Aria", "Eau": "Acqua"}, "pt": {"Feu": "Fogo", "Terre": "Terra", "Air": "Ar", "Eau": "Água"}}[lang]))};
  const modalityNames: Record<string, string> = ${JSON.stringify(({"en": {"Cardinal": "Cardinal", "Fixe": "Fixed", "Mutable": "Mutable"}, "es": {"Cardinal": "Cardinal", "Fixe": "Fija", "Mutable": "Mutable"}, "de": {"Cardinal": "Kardinal", "Fixe": "Fix", "Mutable": "Veränderlich"}, "it": {"Cardinal": "Cardinale", "Fixe": "Fissa", "Mutable": "Mutevole"}, "pt": {"Cardinal": "Cardinal", "Fixe": "Fixa", "Mutable": "Mutável"}}[lang]))};
  return ${JSON.stringify(({"en": "Your dominant {element} energy colors the overall tone of your chart, while the {modality} modality describes your natural way of moving forward. Together they form one of the most visible signatures of your chart.", "es": "Tu energía dominante de {element} colorea el tono general de tu carta, mientras que la modalidad {modality} describe tu manera natural de avanzar. Juntas forman una de las firmas más visibles de tu carta.", "de": "Ihre dominante {element}-Energie prägt den Grundton Ihres Horoskops, während die Modalität {modality} Ihre natürliche Art des Vorangehens beschreibt. Gemeinsam bilden sie eine der sichtbarsten Signaturen Ihres Horoskops.", "it": "La tua energia dominante di {element} colora il tono generale del tema, mentre la modalità {modality} descrive il tuo modo naturale di avanzare. Insieme formano una delle firme più visibili del tuo tema.", "pt": "Sua energia dominante de {element} colore o tom geral do seu mapa, enquanto a modalidade {modality} descreve sua maneira natural de avançar. Juntas formam uma das assinaturas mais visíveis do seu mapa."}[lang]))}
...
function createPlanetCardText(
  ...
  const houseText = typeof planet.house === "number"
    ? " " + ${JSON.stringify(l.inword)} + " " + ${JSON.stringify(l.house)} + " " + String(Math.round(planet.house))
    : "";
  return (keys[name] || "") + " " + displayName + " " + ${JSON.stringify(l.inword)} + " " + sign + houseText + ". " + (${JSON.stringify(({"en": "This position shows how this psychological function is expressed concretely in your chart.", "es": "Esta posición muestra cómo se expresa concretamente esta función psicológica en tu carta.", "de": "Diese Position zeigt, wie sich diese psychologische Funktion konkret in Ihrem Horoskop ausdrückt.", "it": "Questa posizione mostra come questa funzione psicologica si esprime concretamente nel tuo tema.", "pt": "Esta posição mostra como essa função psicológica se expressa concretamente no seu mapa."}[lang]))}) + "";
}
`);
